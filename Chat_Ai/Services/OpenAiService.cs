...
namespace Chat_Ai.Services
{
    public class OpenAiService : IOpenAiService
    {
        ...
        public async Task<string> SendMessageAsync(string userMessage, List<Message> history)
        {
            ...
            {
                return "⚠️ OpenAI API Key is not configured. Please add your API key in appsettings.json under OpenAI:ApiKey. You can get a key from https://platform.openai.com/api-keys";
            }
                ...
                if (!response.IsSuccessStatusCode)
                {
                    ...
                    try
                    {
                        ...
                        var errorMsg = doc.RootElement
                            ...
                            .GetProperty("message")
                            .GetString();
                        return $"⚠️ OpenAI API Error: {errorMsg}";
                    }
                    catch
                    {
                        return $"⚠️ OpenAI API Error (HTTP {(int)response.StatusCode}): {responseJson}";
                    }
                }
